% Modified to allow for model matching inputs and to use
% getPositionalIntensity4 function
...
function [X Xdot T] = gillespieQGeneral4(inpGill)
...
% Set model type separately if matching is performed
matching = inpGill.matching;
if matching
    matchingType = 4;
else
    matchingType = 0;
...
% Ensure no confusion in inputs
if matching && fullWave
    error('Cannot handle both matching and fullwave rectification');
end
...
for i = 2:N
    ...
    if xcurr ~= xold
        if ~fullWave && ~matching
            R = getPositionalIntensity4(xcurr, modelType, intenParams, nPos, Rold, xold, nStates);
        elseif fullWave
            % Fullwave rectification
            R = getPositionalIntensity4(xcurr, fullWaveType, intenParams, nPos, Rold, xold, nStates);
        else
            % Model matching case
            R = getPositionalIntensity4(xcurr, matchingType, intenParams, nPos, Rold, xold, nStates);
        end
    else
        R = Rold;
    end
