...
% Variables to control the execution of certain cells and saving data
cell1 = 1;
cell2 = 0;
savedata = 0;

% Booleans to control the type of simulation performed
testCon = 0;    % removes some connections on the MC
fullWave = 0;   % fullwave rectifies the stimuli
interleave = 0; % interleaves normal and reverse stimuli
matching = 0;   % runs cases in which models match (no intensity switch)
blackDot = 0;   % controls the dot type for matching
wantDelay = 0;  % runs additional MSE calculations with delays
...
if cell1
    ...
    i0 = 10*0.001;
    if matching
        % Assign the dot intensity for the matching case
        if ~blackDot
            % As only white can have ic > i0 for better estimates
            ic = 1*i0;
        else
            % Black dot sets a specific contrast
            ic = -i0;
        end
    else
        % For flashing ic <= i0
    ...
    [mseDirecDel msePosDel mseDirec msePos lamInp] = snyderDelNoDel3(X, T, modType, delay, intenInp, nPos, nStates, Q, lamInp, modVal, wantDelay);
end
