% Modified to allow for model matching
...
function R = getPositionalIntensity4(xcurr, modelType, intenParams, nPos, Rold, xold, nStates)
...
%             R(posidOld) = i0 - ic;
    ...
    case 4
        % Matching case involves a white dot moving on a gray background
        i0 = intenParams.i0;
        ic = intenParams.ic;        
        
        % Apply the background intensity and make current intensity white
        R = i0*R;
        posid = getPosfromState(xcurr, nPos, redPos);
        R(posid) = R(posid) + ic;
