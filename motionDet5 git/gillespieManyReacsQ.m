...
function [X Xdot T] = gillespieManyReacsQ(inpGill)
...
% Extra inputs for the Q matrix rate method
Q = inpGill.Q;
restrict = inpGill.restrict;
useQ = inpGill.useQ;
...
for i = 2:N
    ...
    % Obtain the complete set of reaction rates based on method
    if ~useQ
        % Get reaction rates from set formula
        rdot = getGenReacRates(xhist, rprev, x, r_const, nReacs, reacType,...
            molecType, crossType, SlimSet, bulk);
    else
        % Get reaction rates from Q matrix and lam = alpha*x1 assuming that
        % alpha is r_const(end) <------------------------------------------
%         [rdotx1 bulk] = getRatesQMx(Q, x(1), restrict);
        rdotx1 = getRatesQMx2(Q, x(1), nReacs-1);
