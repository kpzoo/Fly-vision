...
function qKL = plotPosteriorIntensity4(T, q, lam, lamcap, x1, x1cap, S, kg, kb, kd, reldyn, save_sim, plot_on)
...
[nr nc] = size(q);
if length(lam) ~= length(lamcap) || length(lamcap) ~= length(T)
...
if tit_id
    title(['Population comparison at [kg kb kd relspd] = ' num2str(kg) ' '...
        num2str(kb) ' ' num2str(kd) ' ' num2str(reldyn)]);
else
...
if save_sim
    if plotname
        saveas(gcf, ['snyderE_' num2str(kg) '_' num2str(kb) '_' num2str(kd)]);
    else
