...
% Calculate full error statistics at ODE points <-----=-- the normalised
% calculations assume linear encoding
[lamn lamcapn Tn fullstats] = getIntensityStatsFull(lam, lamcapFull, Tset, T, Qset);
kg = params.kgain;
fullstats.meanErrScale = fullstats.meanErr/kg;
fullstats.mseErrScale = fullstats.mseErr/(kg^2);
fullstats.varErrScale = fullstats.varErr/(kg^2);
psi = fullstats.mseErrScale/rawbnd;
disp(['Normalised complete stats are: [<e> var(e) <e^2>] = ' num2str(fullstats.meanErrScale) ' '...
    num2str(fullstats.varErrScale) ' ' num2str(fullstats.mseErrScale)]);
disp('********************************************************************');
disp(['The ratio of mse to bound is ' num2str(psi)]);
disp(['The ratio of N2 to N1 is ' num2str(Nratio)]);
disp('********************************************************************');

% Plot comparison of lam and lamcap with ODE points
figure;
stairs(Tn, [lamn lamcapn]);
xlim([min(Tn) max(Tn)])
legend('actual intensity', 'estimate', 'location', 'best');
xlabel('time');
ylabel('modulating intensity');
title('Comparison of intensity and estimate with ODE data');

% Obtain and plot lamcap with ODE points with event times of x2
[Tevent perc] = getEventTimes(T, X(:, 2), 'birth');
figure;
stairs(Tn, lamcapn);
...
[x1n x1capn Tn fullstats] = getIntensityStatsFull(x1, x1Set, Tset, T, Qset);
plotPosteriorIntensity5(Tn, q, lamn, lamcapn, x1n, x1capn, S, kg, kb, kd, reldyn, save_sim, plot_on1);
