...
function outFil = filterSnyderHybrid5(lamcap0, q0, B, T, S, x1cap0, coeff, lenS, birStr2)

% Boolean for debugging and switch to ode simple from numerical
debug_on = 0;
ode_sw = 0;
...
Tset = cell(1, 1);
Cset = cell(1, 1);
options = odeset('NonNegative', 1:lenS, 'Refine', 10);
...
%     options = odeset(options, 'MaxStep', 0.01*dt(i));
    if ~ode_sw
        [tset, qset] = ode113(@(ts, y) odeSnyderOffline5(ts, y, C),...
            [T(i-1) T(i)], q(i-1, :)', options);
    else
        
        [tset, qset] = ode113(@(ts, y) odeSimpleState(ts, y, 10, 1000), [T(i-1) T(i)], q(i-1, :)', options);
    end
