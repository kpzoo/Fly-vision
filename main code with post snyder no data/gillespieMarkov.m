% Modified from gillespieFilter4 to include an option for a Markov chain
% with fixed finite state space and state dependent birth and death rates
...
function [X Xdot T] = gillespieMarkov(inpGill)
...
% Extra Markov rate and state space extrema variables: [min max] for x1
Slim = inpGill.Slim;
...
for i = 2:N
    ...
    for j = 1:len
        ...
        switch(deaType(j))
            ...
            case 2
                % Linear Markov chain deaths - on x1
                rdot(2*j) = (x(j) - Slim(1))*r_const(2*j);
        ...
        switch(birType(j))
            ...
            case 7
                % Linear Markov chain births - on x1
                rate_x1 = (Slim(2) - x(j))*r_const(2*j - 1);
                rdot(2*j - 1) = rate_x1;
