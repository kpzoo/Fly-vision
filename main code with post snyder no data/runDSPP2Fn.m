% Modified to work with postBatchSnyder by only loading outGil from simname
% to prevent input overwrite and params

% Modified to include skip_Snyder and the rate and molec structured
% variables in the output outB for use with batch
...
% Run Snyder filter if boolean set
if ~skip_Snyder
    % Clear workspace and load data file
    % clear all
    clc
    
    % Load specific variables to prevent overwrite of data from postBatchSnyder
    inpBatch = inpB;
    load(inpB.simname, 'outGil');
    load(inpB.simname, 'params');
    load(inpB.simname, 'inpB');
    coeff = inpB.coeff;
    inpTemp = inpB;
    inpB = inpBatch;
...
if ~inpB.skip_Snyder
    % Assign output from Snyder filter
    ...
    outB.x2rateMean = x2rateMean;
    outB.kdeath = params.kdeath;
    outB.kbirth = params.kbirth;
    outB.kgain = params.kgain;
else
    % Filler output and alternate assignments
    outB.x2rateMean = outGil.rate.mean(3);
    outB.x1Stats = 0;
    outB.lamstats = 0;
end

% Assign outputs from Gillespie simulation
outB.rate = outGil.rate;
outB.molec = outGil.molec;
